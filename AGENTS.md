# 🤖 AGENTS.md - AI Developer Guide for Master Cliper

## 📌 Project Overview
**Master Cliper** is a desktop application that automates the creation of short-form content (TikTok, Reels, Shorts) from long-form YouTube videos. It leverages AI (GPT-4, Whisper) for highlight detection and captioning, and Computer Vision (OpenCV) for smart cropping.
...
1. **Frontend (GUI)**:
   - Entry point: `app.py` (Main `MasterCliper` class).
   - Pages: Located in `pages/` (e.g., `browse_page.py`, `settings_page.py`).
   - Navigation: Managed by `MasterCliper.show_page`.
